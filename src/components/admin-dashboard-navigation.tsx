...
export function AdminSidebar({ dashboard }: { dashboard: PortalDashboardData }) {
  ...
  const roleLabel = dashboard.profile ? roleLabels[dashboard.profile.role] || "Admin" : "Admin";
        ...
        <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
          ...
          <div className="mt-4 grid gap-2">
            <Link
              ...
              className="inline-flex min-h-10 w-full items-center justify-center rounded-xl border border-white/15 bg-white/5 px-3 text-sm font-semibold text-white transition hover:bg-white/10"
            >
              Manage access
            </Link>
            <AdminSessionButton />
  ...
  return (
    <header className="sticky top-0 z-40 border-b border-slate-200 bg-white/95 backdrop-blur lg:hidden">
      <div className="flex h-16 items-center justify-between gap-4 px-4 sm:px-6">
        <Link href="/admin-dashboard" aria-label="HouseOfDev admin home">
          <BrandLogo />
        </Link>
