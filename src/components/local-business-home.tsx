...
function ProjectMockup({ project }: { project: HomeDemoProject }) {
  return (
    <div className="max-w-full overflow-hidden rounded-lg border border-white/10 bg-white/[0.04]">
      <div className="flex h-10 min-w-0 items-center gap-2 border-b border-white/10 px-4">
        <span className="h-2.5 w-2.5 rounded-full bg-red-400" />
        <span className="h-2.5 w-2.5 rounded-full bg-amber-400" />
        <span className="h-2.5 w-2.5 rounded-full bg-emerald-400" />
        <span className="ml-auto shrink-0 rounded bg-white/10 px-2 py-1 text-[10px] font-semibold uppercase tracking-[0.14em] text-zinc-400">
          Demo Project
        </span>
      </div>
    ...
    <div className="mt-8 grid gap-4 lg:grid-cols-3">
      {packages.map((plan) => {
        ...
        return (
          ...
          >
            {plan.isPopular ? (
              <span className="mb-3 inline-flex rounded-full bg-emerald-400 px-3 py-1 text-[11px] font-bold uppercase tracking-[0.14em] text-slate-950 sm:absolute sm:right-4 sm:top-4 sm:mb-0">
                Most Popular
              </span>
            ) : null}
            <h3 className="text-xl font-semibold text-white sm:pr-28">{plan.name}</h3>
...
export function LocalBusinessHome({ content }: { content: HomeContent }) {
  ...
  return (
    <div className="overflow-x-clip bg-[#050816] text-white">
      <section id="hero" className="relative isolate overflow-hidden bg-[#050816] text-white">
        ...
        <div className="mx-auto grid min-h-[calc(100svh-4rem)] max-w-7xl items-center gap-8 px-4 py-12 sm:px-6 lg:grid-cols-[0.9fr_1.1fr] lg:gap-10 lg:px-8 lg:py-20">
          <motion.div {...fadeUp()} className="min-w-0">
            ...
            <h1 className="mt-5 max-w-3xl break-words text-3xl font-semibold leading-tight tracking-normal sm:text-5xl lg:text-6xl">
          ...
          >
            ...
            >
              <div className="rounded-lg border border-white/10 bg-[#0B1020] p-4">
                <div className="flex min-w-0 items-center gap-2 border-b border-white/10 pb-3">
                  <span className="h-2.5 w-2.5 rounded-full bg-red-400" />
                  <span className="h-2.5 w-2.5 rounded-full bg-amber-400" />
                  <span className="h-2.5 w-2.5 rounded-full bg-emerald-400" />
                  <span className="ml-auto max-w-[55%] truncate rounded bg-white/10 px-2 py-1 text-xs text-zinc-400">clinic-demo.local</span>
                </div>
                <div className="grid gap-4 pt-4 md:grid-cols-[1fr_170px]">
                  <div className="min-w-0 rounded-lg bg-gradient-to-br from-sky-300/20 to-emerald-300/10 p-4 sm:p-5">
                    <p className="text-xs font-semibold uppercase tracking-[0.16em] text-sky-200">Clinic Website Preview</p>
                    <h2 className="mt-3 break-words text-xl font-semibold sm:text-2xl">Book appointments faster</h2>
                  ...
                  <div className="mx-auto w-full max-w-[150px] rounded-lg border border-white/15 bg-black p-2">
                    <div className="min-h-[300px] rounded-lg bg-white p-3 text-slate-950">
      ...
      <section id="services" className="overflow-hidden bg-[#050816] py-12 text-white sm:py-16">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <motion.div {...fadeUp()} className="max-w-2xl">
            <p className="text-sm font-semibold uppercase tracking-[0.16em] text-emerald-300">Services</p>
            <h2 className="mt-3 break-words text-2xl font-semibold sm:text-4xl">Everything a local business website needs</h2>
          </motion.div>
      ...
      <section id="work" className="overflow-hidden bg-[#050816] py-12 text-white sm:py-16">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <motion.div {...fadeUp()} className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
            <div className="max-w-2xl">
              ...
              <h2 className="mt-3 break-words text-2xl font-semibold sm:text-4xl">Compact demo projects for common local businesses</h2>
            </div>
          ...
          <div className="mt-8 grid gap-4 lg:grid-cols-3">
            {content.demoProjects.slice(0, 3).map((project, index) => (
              <motion.article key={project.slug} {...slideLeft(index * 0.05)} className="min-w-0 rounded-lg border border-white/10 bg-[#0B1020] p-4">
      ...
      <section id="process" className="overflow-hidden bg-[#050816] py-12 text-white sm:py-16">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <motion.div {...fadeUp()} className="max-w-2xl">
            <p className="text-sm font-semibold uppercase tracking-[0.16em] text-emerald-300">Process</p>
            <h2 className="mt-3 break-words text-2xl font-semibold sm:text-4xl">A simple launch path</h2>
          </motion.div>
      ...
      <section id="pricing" className="overflow-hidden bg-[#050816] py-12 text-white sm:py-16">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <motion.div {...fadeUp()} className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
            <div className="max-w-2xl">
              ...
              <h2 className="mt-3 break-words text-2xl font-semibold sm:text-4xl">Clear packages for serious local businesses</h2>
            </div>
            <div className="grid w-full grid-cols-2 rounded-md border border-white/10 bg-white/[0.04] p-1 sm:inline-grid sm:w-auto">
              {[
                ["one-time", "One-Time Website", "One-Time"],
                ["monthly", "Website + Monthly Maintenance", "Monthly Care"],
              ].map(([value, label, shortLabel]) => (
                <button
                  key={value}
                  type="button"
                  onClick={() => setPricingMode(value as "one-time" | "monthly")}
                  className={`min-h-10 rounded px-3 text-center text-xs font-semibold transition sm:px-4 sm:text-sm ${
      ...
      <section id="contact" className="overflow-hidden bg-[#050816] px-4 pb-28 pt-12 text-white sm:px-6 sm:pt-16 md:pb-16 lg:px-8">
        <div className="mx-auto grid max-w-7xl gap-8 lg:grid-cols-[0.9fr_1.1fr]">
          <motion.div {...fadeUp()}>
            <p className="text-sm font-semibold uppercase tracking-[0.16em] text-emerald-300">Contact</p>
            <h2 className="mt-3 break-words text-2xl font-semibold sm:text-4xl">Ready to Bring Your Business Online?</h2>
            ...
            <div className="mt-6 grid gap-3 text-sm text-zinc-300">
              <a href={whatsappUrl} className="flex min-w-0 items-center gap-2 break-words transition hover:text-emerald-300">
                <MessageCircle className="h-4 w-4 text-emerald-300" />
                WhatsApp: +91 88384 01597
              </a>
              <a href="mailto:arasanredt@gmail.com" className="flex min-w-0 items-center gap-2 break-all transition hover:text-sky-300">
          ...
          <motion.form action={formAction} {...slideLeft(0.05)} className="grid min-w-0 gap-4 rounded-lg border border-white/10 bg-[#0B1020] p-4 shadow-2xl shadow-black/20 sm:p-5">
            ...
            <div className="grid gap-3 lg:grid-cols-[1fr_auto] lg:items-center">
              <p className={state.ok ? "min-w-0 break-words text-sm font-semibold text-emerald-300" : "min-w-0 break-words text-sm text-zinc-400"}>
