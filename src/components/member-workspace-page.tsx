...
function statusTone(value: string) {
  ...
  if (status.includes("complete") || status.includes("approved")) return "bg-emerald-50 text-emerald-700 border-emerald-200";
  if (status.includes("progress") || status.includes("review")) return "bg-blue-50 text-blue-700 border-blue-200";
  if (status.includes("block") || status.includes("urgent")) return "bg-red-50 text-red-700 border-red-200";
...
function ProjectsSection({ kind, data }: { kind: Kind; data: MemberWorkspaceData }) {
  return <div className="grid gap-4 xl:grid-cols-2">{data.projects.length ? data.projects.map((project) => <ProgressCard key={project.id} project={project} href={kind === "employee" ? "/employee-portal/updates" : "/portal/updates"} />) : <p className="rounded-3xl border border-dashed border-slate-300 bg-white p-8 text-sm text-slate-500">No projects are connected to this account yet.</p>}</div>;
}

function TasksSection({ data }: { data: MemberWorkspaceData }) {
  return <div className="grid gap-4">{data.tasks.length ? data.tasks.map((task) => <article key={task.id} className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm"><div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between"><div><p className="text-xs font-bold uppercase tracking-[0.14em] text-blue-600">{task.projectTitle}</p><h2 className="mt-2 text-lg font-semibold">{task.title}</h2><p className="mt-2 text-sm leading-6 text-slate-500">{task.description || "No extra description was added."}</p><p className="mt-3 text-xs text-slate-500">Priority: {task.priority} · Due {date(task.dueDate)}</p></div><form action={updateEmployeeTaskStatus} className="flex min-w-56 gap-2"><input type="hidden" name="taskId" value={task.id} /><select name="status" defaultValue={task.status} className="min-h-10 flex-1 rounded-xl border border-slate-300 bg-white px-3 text-sm"><option value="todo">To do</option><option value="in_progress">In progress</option><option value="review">Review</option><option value="blocked">Blocked</option><option value="completed">Completed</option></select><button type="submit" className="min-h-10 rounded-xl bg-slate-950 px-4 text-sm font-semibold text-white hover:bg-blue-700">Save</button></form></div></article>) : <p className="rounded-3xl border border-dashed border-slate-300 bg-white p-8 text-sm text-slate-500">No tasks are assigned to this account.</p>}</div>;
}
...
function UpdatesSection({ kind, data }: { kind: Kind; data: MemberWorkspaceData }) {
  return <div className="grid items-start gap-6 xl:grid-cols-[minmax(0,1fr)_380px]"><section className="grid gap-4">{data.updates.length ? data.updates.map((update) => <article key={update.id} className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-[0.14em] text-blue-600">{update.projectTitle}</p><h2 className="mt-2 text-lg font-semibold">{update.title}</h2></div><span className="text-xs text-slate-500">{update.author} · {date(update.createdAt)}</span></div><p className="mt-4 whitespace-pre-line text-sm leading-6 text-slate-600">{update.body}</p></article>) : <p className="rounded-3xl border border-dashed border-slate-300 bg-white p-8 text-sm text-slate-500">No project updates have been posted.</p>}</section>{kind === "employee" ? <form action={submitEmployeeProjectUpdate} className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm xl:sticky xl:top-6"><h2 className="text-xl font-semibold">Daily project review</h2><p className="mt-2 text-sm leading-6 text-slate-500">Share what changed today so the admin and client can understand progress.</p><label className="mt-5 block"><span className="text-sm font-semibold">Project</span><select name="projectId" className={input} required defaultValue=""><option value="" disabled>Select project</option>{data.projects.map((project) => <option key={project.id} value={project.id}>{project.title}</option>)}</select></label><label className="mt-4 block"><span className="text-sm font-semibold">Update title</span><input name="title" className={input} placeholder="Homepage development completed" required /></label><label className="mt-4 block"><span className="text-sm font-semibold">Completed today</span><textarea name="completed" className={`${input} min-h-24 py-3`} required /></label><label className="mt-4 block"><span className="text-sm font-semibold">Next work</span><textarea name="next" className={`${input} min-h-20 py-3`} /></label><label className="mt-4 block"><span className="text-sm font-semibold">Blocker, if any</span><textarea name="blocker" className={`${input} min-h-20 py-3`} /></label><button type="submit" className="mt-5 min-h-11 w-full rounded-xl bg-blue-700 px-5 text-sm font-semibold text-white hover:bg-violet-700">Share daily update</button></form> : null}</div>;
}

function ChangeRequestsSection({ data }: { data: MemberWorkspaceData }) {
  return <div className="grid items-start gap-6 xl:grid-cols-[minmax(0,1fr)_400px]"><section className="grid gap-4">{data.tickets.length ? data.tickets.map((ticket) => <article key={ticket.id} className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm"><div className="flex items-start justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-[0.14em] text-blue-600">{ticket.projectTitle}</p><h2 className="mt-2 font-semibold">{ticket.subject}</h2></div><span className={`rounded-full border px-3 py-1 text-xs font-bold capitalize ${statusTone(ticket.status)}`}>{ticket.status}</span></div><p className="mt-4 text-sm leading-6 text-slate-600">{ticket.body}</p><p className="mt-4 text-xs text-slate-400">Priority {ticket.priority} · {date(ticket.createdAt)}</p></article>) : <p className="rounded-3xl border border-dashed border-slate-300 bg-white p-8 text-sm text-slate-500">No change requests have been submitted.</p>}</section><form action={submitClientProjectChangeRequest} className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm xl:sticky xl:top-6"><h2 className="text-xl font-semibold">Request a change</h2><p className="mt-2 text-sm leading-6 text-slate-500">Describe the business change you need. Avoid technical detail; the team will translate it into tasks.</p><label className="mt-5 block"><span className="text-sm font-semibold">Project</span><select name="projectId" className={input} required defaultValue=""><option value="" disabled>Select project</option>{data.projects.map((project) => <option key={project.id} value={project.id}>{project.title}</option>)}</select></label><label className="mt-4 block"><span className="text-sm font-semibold">Short subject</span><input name="subject" className={input} placeholder="Change appointment form fields" required /></label><label className="mt-4 block"><span className="text-sm font-semibold">What should change?</span><textarea name="body" className={`${input} min-h-28 py-3`} required /></label><label className="mt-4 block"><span className="text-sm font-semibold">Priority</span><select name="priority" className={input} defaultValue="normal"><option value="low">Low</option><option value="normal">Normal</option><option value="high">High</option><option value="urgent">Urgent</option></select></label><button type="submit" className="mt-5 min-h-11 w-full rounded-xl bg-slate-950 px-5 text-sm font-semibold text-white hover:bg-blue-700">Send change request</button></form></div>;
}

export async function MemberWorkspacePage({ kind, section, authError, authNotice }: { kind: Kind; section: Section; authError?: string; authNotice?: string }) {
  const data = await getMemberWorkspaceData(kind);
  if (data.dashboard.mode !== "live") return <PortalAccessGate dashboard={data.dashboard} authError={authError} authNotice={authNotice} />;
