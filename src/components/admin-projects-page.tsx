...
import { ArrowRight, Filter, FolderKanban, Users } from "lucide-react";
          ...
          <main className="min-w-0 p-4 sm:p-6 xl:p-8">
            <header className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-7">
              <div className="flex flex-col gap-5 xl:flex-row xl:items-end xl:justify-between">
                <div className="max-w-4xl">
                  ...
                  <p className="mt-3 max-w-3xl text-sm leading-6 text-slate-600 sm:text-base">Track each client project, assigned employee, progress percentage, task pressure, and update history.</p>
                </div>
            ...
            <div className="mt-6 grid gap-4 xl:grid-cols-2">
              {projects.length ? projects.map((project) => (
                ...
                <div className="xl:col-span-2 rounded-3xl border border-dashed border-slate-300 bg-white p-8 text-center"><FolderKanban className="mx-auto h-8 w-8 text-slate-400" /><h2 className="mt-4 text-lg font-semibold">No matching projects</h2><p className="mt-2 text-sm text-slate-500">This client or employee is not connected to a project yet.</p><Link href="/admin-dashboard/projects" className="mt-5 inline-flex min-h-10 items-center rounded-xl bg-slate-950 px-4 text-sm font-semibold text-white">View all projects</Link></div>
              )}
            </div>
          </main>
        </div>
